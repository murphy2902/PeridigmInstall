...
${BUILD_DIR}/${GCC}/contrib/download_prerequisites
...
cp -vf ${BUILD_DIR}/${NETCDF}/include/netcdf_par.h ${ROOT_DIR}/include/
...
export LDFLAGS="${LDFLAGS} -L${ROOT_DIR}/lib -Ldl"
...
export CXXFLAGS="-std=gnu++0x -Wall -g"
export LDFLAGS="-L${ROOT_DIR}/lib -Ldl"
export PATH=${PATH}:${ROOT_DIR}/bin

PERIDIGM=`ls ${BUILD_DIR} | grep -m 1 "Peridigm"`

if test -d ${BUILD_DIR}/${PERIDIGM}-bin ; then
   rm -r ${BUILD_DIR}/${PERIDIGM}-bin
fi

mkdir -vp ${BUILD_DIR}/${PERIDIGM}-bin
cd ${BUILD_DIR}/${PERIDIGM}-bin

cmake \
-D CMAKE_BUILD_TYPE:STRING=Release \
-D Trilinos_DIR:PATH=${ROOT_DIR}/lib/cmake/Trilinos/ \
-D CMAKE_C_COMPILER:STRING=${ROOT_DIR}/bin/mpicc \
-D CMAKE_CXX_COMPILER:STRING=${ROOT_DIR}/bin/mpicxx \
-D BOOST_ROOT=${ROOT_DIR} \
-D Boost_INCLUDE_DIRS:PATH=${ROOT_DIR}/include \
-D Boost_LIBRARY_DIRS:PATH=${ROOT_DIR}/lib \
-D HDF5_LIBRARY_DIRS:PATH="${ROOT_DIR}/lib" \
-D HDF5_INCLUDE_DIRS:PATH="${ROOT_DIR}/include" \
-D Netcdf_INCLUDE_DIRS:PATH=${ROOT_DIR}/include \
-D Netcdf_LIBRARY_DIRS:PATH=${ROOT_DIR}/lib \
-D CMAKE_CXX_FLAGS:STRING="-O2 -Wall -ansi -pedantic -Wno-long-long -ftrapv -Wno-depracated -std=gnu++0x" \
${BUILD_DIR}/${PERIDIGM}

make | tee ${REPORT_DIR}/peridigm_make_report.log
make test | tee ${REPORT_DIR}/peridigm_check_report.log
